...
//alterar e adicionar para excluir pf e pessoa_id através do pessoa_id
   if (!cliente_id){
    ...
    const query = 'delete from cliente cl where cl.cliente_id = ?'

    db.execute(query, [cliente_id], (erroDelete, results) => {
        if (erroDelete){
            console.error('Erro ao excluir cliente. Verifique!', erroDelete);
            return res.status(500).json({ 
                success: false,
